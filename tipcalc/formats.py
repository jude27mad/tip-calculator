...
_DEFAULT_ROUNDING = ROUND_HALF_EVEN
...
def quantize_amount(
    ...
    *,
    ...
    """Quantize ``value`` to the desired ``step`` with the given rounding mode."""
    return value.quantize(step, rounding=rounding)


def to_cents(value: Decimal, *, rounding: str = _DEFAULT_ROUNDING) -> Decimal:
    """Round a Decimal to two fractional digits using the configured rounding mode."""
    return quantize_amount(value, step=CENT, rounding=rounding)
...
def fmt_percent(value: Decimal) -> str:
    ...
    q = quantize_amount(value, step=PERCENT_STEP, rounding=ROUND_HALF_UP)
...
def print_results(
    *,
    ...
    def _money(value: Decimal) -> str:
        return fmt_money(value, symbol=sym, currency=currency, locale=locale)

    lines.append(f"Subtotal (pre-tax): {_money(bill_before_tax)}")
    if tax_percent is not None:
        lines.append(f"Tax: {fmt_percent(tax_percent)}% of {_money(bill_before_tax)} -> {_money(tax_amount)}")
    else:
        lines.append(f"Tax: {_money(tax_amount)} (entered amount)")
    lines.append(f"After-tax total (before tip): {_money(original_total)}")
    lines.append(f"Tip ({tip_base_label} at {fmt_percent(tip_percent)}%): {_money(tip)}")
    lines.append(f"Total with tip: {_money(final_total)}")
    lines.append(f"Breakdown: {_money(bill_before_tax)} + {_money(tax_amount)} + {_money(tip)} = {_money(final_total)}")
    if len(per_person) == 1:
        lines.append(f"Each person pays: {fmt_money(per_person[0], symbol=sym, currency=currency, locale=locale)}")
    else:
        shares = ", ".join(_money(p) for p in per_person)
